...
//  Project7
//
//  Created by Samritha Nagesh on 12/2/18.
...
void Player::moveOrAttack(int dir)
{
    ...
    {
        case UP:
            if(m_arena->numZombiesAt(row()-1, col()) > 0){ //Attack zombie next to player
                m_arena->attackZombieAt(row()-1, col(), dir);
            }
            else if(m_row > 1){ //No zombies next to player
                m_row--;
            }
            break;
        case DOWN:
        ...
        case LEFT:
            if(m_arena->numZombiesAt(row(), col()-1) > 0)
                m_arena->attackZombieAt(row(), col()-1, dir);
            else if(m_col > 1)
                m_col--;
            break;
        case RIGHT:
...
bool Arena::addZombie(int r, int c) //POSSIBLY FIX?
{
...
bool Arena::attackZombieAt(int r, int c, int dir)
{
    ...
    if(numZombiesAt(r,c) > 0){
        for(int i = 0; i < m_nZombies; i++){
            if(m_zombies[i] != nullptr && m_zombies[i]->row() == r && m_zombies[i]->col() == c){
                if(m_zombies[i]->getAttacked(dir))
                {
                    Zombie* temp = m_zombies[i];
                    m_zombies[i] = m_zombies[m_nZombies-1];
                    m_zombies[m_nZombies-1] = temp;
